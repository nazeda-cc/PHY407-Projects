...
while delta > target:
    ...
    for i in range(1, M):
        for j in range(1, M):
            if i == 0 or i == M or j == 0 or j == M:  # grounded bdry condition
                phi[i, j] = phi[i, j]
...
print('Gauss-Seidel without overrelaxation took %f seconds' % (time_clock))
