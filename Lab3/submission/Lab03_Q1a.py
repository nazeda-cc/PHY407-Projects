...
#Calculate and relative error from scipy.special.dawsn
error_trap = []
error_simp = []
error_gauss = []
...
#Calculated and plot the calculated error with textbook eqns.
...
for i in n:
    N_1 = 2**i
    N_2 = 2**(i+1)
    calculated_error_trap.append(abs(Daws(4, N_1, 'Trap') - Daws(4, int(N_2/4), 'Trap')) / 3)
    calculated_error_simp.append(abs(Daws(4, N_1, 'Simp') - Daws(4, int(N_2/4), 'Simp')) / 15)
