...
delta = 1e3/3.154e7  # desired accuracy in positional error per week (in seconds)
...
# visualize the orbit of the Earth
...
plt.legend(loc=1)
plt.title('Simulated Earth\'s orbit')
...
tpoints = np.arange(0, 4.73e+10, H)   # time period is 
xpoints, ypoints = bulirsch(r, tpoints, H, delta)

# visualize the orbit of the Pluto
plt.subplot(1, 2, 2)
plt.plot(np.array(xpoints)/AU, np.array(ypoints)/AU, lw=3)
plt.grid()
plt.xlabel('X (AU)')
...
plt.legend(loc=1)
plt.title('Simulated Pluto\'s orbit')